...
describe("Testing of the public functions of the promisesEngine", function() {
	...
	describe("Tests actions without params", function() {
		it("Makes a batch of actions [pack of 2, no delay, no retry]", function() {
			var actions = [action.working3, action.working1, action.working2];
			var params = [];

			var promiseResult = promisesEngine.batch(actions, params, 2, true, 0, 0, false, 0);
		...
		it("Makes a batch of actions [pack of 1, no delay, retry]", function() {
			var actions = [action.working3, action.failingOnFirstCall];
			var params = [];

			var promiseResult = promisesEngine.batch(actions, params, 1, true, 1, 0, true, 0);
			Q.isPromise(promiseResult).should.equal(true);
			return promiseResult.then(function(results){
				//console.log("results : "+results);
				results.should.be.an('array');
		...
		it("Makes a batch of actions with retry [pack of 2, no delay, retry]", function() {
			...
			var promiseResult = promisesEngine.batch(actions, params, 2, true, 1, 0, true, 0);
			Q.isPromise(promiseResult).should.equal(true);
			return promiseResult.then(function(results){
				//console.log("results : "+results);
				results.should.be.an('array');
		...
		it("Makes a chain of actions", function(){
			...
			var promiseResult = promisesEngine.chain(actions, "initialParam");
			Q.isPromise(promiseResult).should.equal(true);
		...
		it("Does retry an action", function(){
			var promiseResult = promisesEngine.retry(action.failingOnFirstCall, null);
			Q.isPromise(promiseResult).should.equal(true);
			return promiseResult.then(function(result){
				//console.log("result : "+result);
		...
		it("Does run in parallel actions", function(){
			...
			var promiseResult = promisesEngine.parallelize(actions, params, true, 1, 0, true);
			Q.isPromise(promiseResult).should.equal(true);
			return promiseResult.then(function(results){
				//console.log("results : "+results);
				results.should.be.an('array');
	...
	describe("Tests actions without params", function() {
		it("Makes a batch of actions [pack of 2, no delay, no retry]", function() {
			var actions = [action.working3, action.working1, action.working2];
			var params = [["plop","plip"], ["plop"], ["plop"]];
